...
import (
	...
	"github.com/netclave/bluetooth-library/linux"
...
func NewDevice(fd int, opts ...Option) (Device, error) {
	d := &device{
	...
	d.Option(opts...)
	h, err := linux.NewHCI(fd, d.devID, d.chkLE, d.maxConn)
	if err != nil {
	...
	log.Println("New linux device created")
	return d, nil
}
...
func (d *device) CloseSocket() error {
