...
import (
	...
	"github.com/netclave/bluetooth-library/linux/cmd"
	"github.com/netclave/bluetooth-library/linux/evt"
)
...
func NewHCI(fd int, devID int, chk bool, maxConn int) (*HCI, error) {
	d, err := newDevice(fd, devID, chk)
	if err != nil {
		log.Println("Error on new device")
		return nil, err
	}
	c := cmd.NewCmd(d)
	e := evt.NewEvt()

	log.Println("After new EVT")
	...
	log.Println("Before main loop")

	go h.mainLoop()

	log.Println("At the end of NewHCI")

	return h, nil
}

func (h *HCI) ResetDevice() {
	log.Println("Before reset device")
	h.resetDevice()
	log.Println("After reset device")
}

func (h *HCI) Close() error {
	log.Println("Closing HCI Linux")
...
func (h *HCI) resetDevice() error {
	...
	log.Println("Sequences in the command")
	log.Println(len(seq))
