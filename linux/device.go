...
import (
	...
	"github.com/netclave/bluetooth-library/linux/gioctl"
...
func newDevice(fd int, n int, chk bool) (*device, error) {
	...
	for i := 0; i < int(req.devNum); i++ {
		...
		if err == nil {
			...
			log.Println(i)
			log.Println("Open socket error: " + err.Error())
		}
	}
...
func newSocket(fd, n int, chk bool) (*device, error) {
	...
	if err := gioctl.Ioctl(uintptr(fd), hciGetDeviceInfo, uintptr(unsafe.Pointer(&i))); err != nil {
		log.Println("Dev info error: " + err.Error())
		return nil, err
	}
	name := string(i.name[:])
...
func (d device) Write(b []byte) (int, error) {
	...
	written, err := syscall.Write(d.fd, b)
